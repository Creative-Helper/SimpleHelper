...
class Pipeline {
  ...
  isTree () {
    ...
    return true */
    return this.nodes.children && this.nodes.children.length > 0
  }
  ...
  getWidthOfTree (index) {
    const node = this.nodes.find(item => item.id === index)
    ...
    for (let i = 0; i < node.children.length; i++) {
      width += this.getWidthOfTree(node.children[i].id)
    }
    return width
  }
  ...
  assignNodeForTree (index, x, y) {
    ...
    for (let i = 0; i < node.children.length; i++) {
      const width = this.getWidthOfTree(node.children[i].id)
      this.assignNodeForTree(node.children[i].id, xx, yy)
      yy += width
    }
  }
  ...
  getLines () {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      ...
      for (let j = 0; j < node.children.length; j++) {
        const edge = node.children[j]
        const child = this.nodes.find(item => item.id === edge.id)
        list.push({
  ...
  topologicalSorting () {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      ...
      const list = this.findParents(this.nodes[i].id)
  ...
  findParents (index) {
    ...
    return arr */
    return this.deepSearch(index, this.nodes, true)
  }

  deepSearch (index, list = [], parent = true) {
    const vector = []
    list.forEach(item => {
      if (item.children) {
        vector.merge(this.deepSearch(index, item.children, parent))
      }
      if (parent && item.parent && item.parent === index) {
        vector.push(item.id)
      }
      if (!parent && item.id === index) {
        vector.push(item.id)
      }
    })
    return vector
  ...
  findChildren (index) {
    ...
    return node.children.map((it) => it.id)
  }
